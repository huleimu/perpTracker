//! # Hyperliquid WebSocket 订阅示例：用户实时资金费率更新 (UserFundings)
//!
//! 该文件展示了如何使用 `hyperliquid-rust-sdk` 的 `InfoClient` 订阅来自 Hyperliquid 的**特定用户的实时资金费率支付/收取事件**。
//!
//! `UserFundings` 订阅是针对一个具体的用户地址的。它的作用是，在每个资金费率结算周期（通常是每小时），
//! 当系统为该用户持有的永续合约仓位进行资金费用结算时，服务器会主动推送一条 `UserFundings` 消息。
//! 这条消息详细记录了该用户在哪个资产上支付或收取了多少资金费用。
//!
//! 这对于需要精确跟踪账户盈亏（PnL）的应用程序来说至关重要，因为资金费用是持仓成本的重要组成部分。
//!
//! 其工作流程与之前的 WebSocket 示例一致：
//! 1.  **创建通道 (Channel)**: 建立一个内存中的消息队列。
//! 2.  **订阅数据流**: 订阅指定用户地址的 `UserFundings` 数据流。
//! 3.  **启动取消任务**: 在后台启动一个独立的异步任务，30秒后自动取消订阅。
//! 4.  **处理数据**: 主任务进入一个循环，持续地从通道中接收并打印服务器推送过来的资金费率结算事件。
//!
//! ## 关于输出的说明
//!
...
async fn main() {
    ...
    // ==================== 中文流程输出开始 ====================
    println!("程序启动：开始执行 Hyperliquid WebSocket 用户资金费率事件订阅流程...");
    println!("-------------------------------------------------");
    // =========================================================
    ...
    // 中文输出：告知用户正在发起订阅
    println!("[步骤 3/4] 正在向服务器订阅该用户的 'UserFundings' (资金费率结算事件) 数据流...");
    // 调用 `subscribe` 方法，发起订阅请求。
    let subscription_id = info_client
        .subscribe(
            // 定义订阅类型为 UserFundings，并提供目标用户地址。
    ...
    // 中文输出：告知用户进入接收循环
    println!("\n[步骤 4/4] 进入主循环，开始接收并打印实时资金费率结算事件... (将持续30秒)");
    ...
    while let Some(Message::UserFundings(user_fundings)) = receiver.recv().await {
        ...
        println!("【资金费率事件】收到新消息: {:?}", user_fundings);
    }
