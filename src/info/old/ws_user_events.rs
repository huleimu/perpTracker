//! # Hyperliquid WebSocket 订阅示例：用户全量事件 (UserEvents)
//!
//! 该文件展示了如何使用 `hyperliquid-rust-sdk` 的 `InfoClient` 订阅来自 Hyperliquid 的**特定用户的全量实时事件**。
//!
//! `UserEvents` 是针对单个用户最全面的数据流。它几乎包含了与该用户账户相关的所有状态变更，可以看作是 `OrderUpdates` 和 `Notification` 的超集。
//! 当被监控的账户发生任何活动时，服务器都会推送一条 `UserEvents` 消息，其内容可能包括：
//! -   **订单更新**: 新订单、成交、取消等（等同于 `OrderUpdates` 的内容）。
//! -   **资金变动**: 资金费率的支付/收取、存提款、内部划转。
//! -   **仓位更新**: 开仓、平仓、加减仓导致仓位变化。
//! -   **保证金更新**: 保证金的增加或减少。
//! -   **强平事件**等。
//!
//! 对于需要维护一个与链上状态完全同步的本地账户模型的复杂交易机器人来说，订阅 `UserEvents` 是最佳选择，因为它提供了最完整、最及时的账户信息。
//!
//! ## 关于输出的说明
//!
...
async fn main() {
    ...
    // ==================== 中文流程输出开始 ====================
    println!("程序启动：开始执行 Hyperliquid WebSocket 用户全量事件订阅流程...");
    println!("-------------------------------------------------");
    // =========================================================
    ...
    // 中文输出：告知用户正在发起订阅
    println!("[步骤 3/4] 正在向服务器订阅该用户的 'UserEvents' (全量用户事件) 数据流...");
    // 调用 `subscribe` 方法，发起订阅请求。
    let subscription_id = info_client
        .subscribe(
            // 定义订阅类型为 UserEvents，并提供目标用户地址。
    ...
    // 中文输出：告知用户进入接收循环
    println!("\n[步骤 4/4] 进入主循环，开始接收并打印实时用户全量事件... (将持续30秒)");
    ...
    while let Some(Message::User(user_event)) = receiver.recv().await {
        ...
        println!("【用户全量事件】收到新消息: {:?}", user_event);
    }
