//! # Hyperliquid WebSocket 订阅示例：实时 Level 2 订单簿 (L2Book)
//!
//! 该文件展示了如何使用 `hyperliquid-rust-sdk` 的 `InfoClient` 订阅来自 Hyperliquid 的**实时 WebSocket L2 订单簿数据流**。
//!
//! L2 订单簿数据流是高频交易和市场深度分析的基石。它提供了市场买卖双方挂单的实时、逐级视图。
//! 每当订单簿的任何一个价位上的数量发生变化（无论是新增、修改还是取消订单），服务器都会主动推送一条
//! 包含变化详情的 `L2Book` 消息。
//!
//! 这对于构建以下应用至关重要：
//! -   高频交易策略
...
//! 2.  **订阅数据流**: 订阅指定资产（"ETH"）的 `L2Book` 数据流。
//! 3.  **启动取消任务**: 在后台启动一个独立的异步任务，30秒后自动取消订阅。
//! 4.  **处理数据**: 主任务进入一个循环，持续地从通道中接收并打印服务器推送过来的 `L2Book` 增量更新。
//!
//! ## 关于输出的说明
//!
...
async fn main() {
    ...
    // ==================== 中文流程输出开始 ====================
    println!("程序启动：开始执行 Hyperliquid WebSocket L2订单簿订阅流程...");
    println!("-------------------------------------------------");
    // =========================================================
    ...
    // 中文输出：告知用户正在发起订阅
    println!("[步骤 3/4] 正在向服务器订阅 'ETH' 资产的 'L2Book' (Level 2 订单簿) 数据流...");
    // 调用 `subscribe` 方法，发起订阅请求。
    let subscription_id = info_client
        .subscribe(
            // 定义订阅类型为 L2Book，并提供目标资产。
            Subscription::L2Book {
                coin: "ETH".to_string(),
            },
    ...
    // 中文输出：告知用户进入接收循环
    println!("\n[步骤 4/4] 进入主循环，开始接收并打印实时L2订单簿数据... (将持续30秒)");
    ...
    while let Some(Message::L2Book(l2_book)) = receiver.recv().await {
        ...
        println!("【实时订单簿】收到 L2Book 更新: {:?}", l2_book);
    }
